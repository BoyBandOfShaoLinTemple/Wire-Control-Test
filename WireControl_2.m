...
h3=plot(time(block_start:block),brake(block_start:block),'g','linewidth',1,'marker','o','MarkerIndices',1:10:block);
ylabel('刹车(%)');   % 纵坐标标签
...
ax.XAxis.MinorTickValues=0:0.2:20;  % 绘制 有数的刻度之间的小刻度， 其中绘制区间是0-20 每隔0.2绘制一个
%set(gca,'XMinorTick','on');
%set(gca,'ticklength',[0.1 1]);
set(gca,'xlim',[0 12]);   % 设置横坐标区间0到12
set(gca,'ylim',[-20 10]); % 设置纵坐标区间-20到8
ylabel('实时速度(km/s)');   % 纵坐标标签
xlabel('Time(s)');% 时间
set(gca,'FontSize',30); % 设置字体大小
%grid on;
legend([h1,h2,h3],'EPS Angle Request', 'EPS Real-time Steering Angle','刹车百分比'); 
title('刹车从0到3每次+0.02');